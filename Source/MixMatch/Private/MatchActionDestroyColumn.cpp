...
#include "MatchActionDestroyColumn.h"
...
/*
*/
UMatchActionDestroyColumn::UMatchActionDestroyColumn()
	: Super()
{
}

bool UMatchActionDestroyColumn::Perform_Implementation(const UBlockMatch* Match, const FMatchActionType& MatchActionType, const AMMBlock* TriggeringBlock)
{
	...
	if (MatchActionType.ActionCategory != EMMMatchActionCategory::DestroysBlocks) {
		UE_LOG(LogMMGame, Error, TEXT("MatchActionDestroyColumn MatchActionType.ActionCategory should be DestroysBlocks."));
		return false;
	}
	...
	if (Grid == nullptr) {
		UE_LOG(LogMMGame, Error, TEXT("MatchActionDestroyColumn::Perform - Cannot get grid from match"));
		return false;
	}
	for (int32 Row = 0; Row < Grid->SizeX; Row++)
	{
		DestroyCoords.X = Row;
